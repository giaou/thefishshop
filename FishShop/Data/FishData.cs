...
{
    private readonly List<FishType> types = [
    new FishType{
    ...
    public IEnumerable<Fish> GetFishes() => fishes;

    public Fish? GetFish(Guid id) => fishes.Find(fish => fish.Id == id);
    ...
    public void RemoveFish(Guid id)
    {
        fishes.RemoveAll(fish => fish.Id == id);
    }

    public IEnumerable<FishType> GetTypes() => types;
    public FishType? GetType(Guid id) => types.Find(type => type.Id == id);
}
